...
# ------------------SOLVER----------------
        ...
        # # ------------------------- Configuration -------------------------
        # execution_id = "example_"  # Replace with your execution_id
        ...
        # # Solver parameters
        # MAX_RUNTIME_HOURS = 1           # max runtime
        # OPTIMALITY_GAP_PERCENT = 5      # allowable gap %
        ...
        #                 logging.warning(f"Duty {index} skipped - all service IDs invalid: {invalid_services}")
        #                 continue
        #             if invalid_services:
        #                 logging.warning(f"Duty {index} has some invalid service IDs: {invalid_services}")

        #             service_assignments[index - 1] = valid_services
        ...
        #     logging.warning(f"Not all services are covered! Missing: {set(services) - servicesInDuties}")
        # else:
        ...
        # # ------------------------- Solve Model with Time/Gap -------------------------
        # solver_executable = '/home/ankit_19591/minotaur/build/bin/mbnb'  # Replace with actual path
        # solver = SolverFactory('mbnb', executable=solver_executable)
        ...
        # # Solver options for time limit and gap
        ...
        # solver.options['--obj_gap_percent'] = OPTIMALITY_GAP_PERCENT
        # solver.options['--time_limit'] = int(MAX_RUNTIME_HOURS * 3600)

        # # Temp log file for solver
        # log_fd, log_path = tempfile.mkstemp(prefix="mbnb_", suffix=".log")
        # os.close(log_fd)

        # try:
        #     logging.info(f"Starting solver with max {MAX_RUNTIME_HOURS} hour(s) and {OPTIMALITY_GAP_PERCENT}% gap")
        ...
        #                 # If the duty is selected
        #                 if abs(model.fPath[path_var].value - 1) <= 1e-6:
        #                     # Write only the service IDs for this duty (no header, no index)
