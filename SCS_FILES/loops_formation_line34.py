...
for i in range(df.shape[0]):
  if df.iloc[i,0] not in added_services and int(df.iloc[i,1]) != 398 and int(df.iloc[i,1]) != 399 and 432 <= hhmm2mins(df.iloc[i,5]) <= 1335:# STEP BACK FOR TIME 07 AM TO 09:49 PM
    if df.iloc[i,4] == "DSTO" and df.iloc[i,6] =='UP' :
      flag = False
      for j in range(i, df.shape[0]):
        if df.iloc[j,2] == "DSTO" and df.iloc[j,6] == 'DN' and int(df.iloc[j,1]) != 398 and int(df.iloc[j,1]) != 399 and df.iloc[j,1] != df.iloc[i,1] and 25 >= timeDiff(df.iloc[i,5], df.iloc[j,3]) >= 0 and df.iloc[j,0] not in added_services:
          DW_to_DSTOJ, DSTO_to_DWI = False,False
          for k in range(i-100,j):
            if df.iloc[k,1] == df.iloc[j,1] and df.iloc[k,6] == 'UP' and df.iloc[k,4] == "DSTO" and 0 <= timeDiff(df.iloc[k,5], df.iloc[j,3]) <= 25:
              DW_to_DSTOJ = True
            if df.iloc[k,1] == df.iloc[i,1] and df.iloc[k,2] == "DSTO" and df.iloc[k,6] == 'DN' and 0 <= timeDiff(df.iloc[i,5], df.iloc[k,3]) <= 25:
              DSTO_to_DWI = True
...
for i in range(df.shape[0]):
  if df.iloc[i,0] not in added_services and 432 <= hhmm2mins(df.iloc[i,5]) <= 1313:
    if df.iloc[i,4] == "VASI" and df.iloc[i,6] == 'DN':
      flag = False
      for j in range(i, df.shape[0]):
        if df.iloc[j,2] == "VASI" and df.iloc[j,6] == 'UP' and df.iloc[j,1] != df.iloc[i,1] and 25 >= timeDiff(df.iloc[i,5], df.iloc[j,3]) >= 0 and df.iloc[j,0] not in added_services:
          LN_to_VASI_rakeJ, VASI_to_LN_rakeI = False, False
          for k in range(i-100,j):
              if df.iloc[k,1] == df.iloc[j,1] and df.iloc[k,4] == "VASI" and df.iloc[k,6] == 'DN' and 0 <= timeDiff(df.iloc[k,5], df.iloc[j,3]) <= 25:
                  LN_to_VASI_rakeJ = True
              if df.iloc[k,1] == df.iloc[i,1] and df.iloc[k,2] == "VASI" and df.iloc[k,6] == 'UP' and 0 <= timeDiff(df.iloc[i,5], df.iloc[k,3]) <= 25:
...
for i in range(df.shape[0]):
  if df.iloc[i,0] not in added_services and int(df.iloc[i,1]) != 398 and int(df.iloc[i,1]) != 399 and 438 <= hhmm2mins(df.iloc[i,5]) <= 1318:
    if df.iloc[i,4] == "NECC" and df.iloc[i,6] == 'DN':
      flag = False
      for j in range(i, df.shape[0]):
        if df.iloc[j,2] == "NECC" and df.iloc[j,6] == 'UP' and int(df.iloc[j,1]) != 398 and int(df.iloc[j,1]) != 399 and df.iloc[j,1] != df.iloc[i,1] and 25 >= timeDiff(df.iloc[i,5], df.iloc[j,3]) >= 0 and df.iloc[j,0] not in added_services:
          NSET_to_NECC_rakeJ, NECC_to_NSET_rakeI = False, False
          for k in range(i-100,j):
              if df.iloc[k,1] == df.iloc[j,1] and df.iloc[k,4] == "NECC" and df.iloc[k,6] == 'DN' and 0 <= timeDiff(df.iloc[k,5], df.iloc[j,3]) <= 25:
                  NSET_to_NECC_rakeJ = True
              if df.iloc[k,1] == df.iloc[i,1] and df.iloc[k,2] == "NECC" and df.iloc[k,6] == 'UP' and 0 <= timeDiff(df.iloc[i,5], df.iloc[k,3]) <= 25:
