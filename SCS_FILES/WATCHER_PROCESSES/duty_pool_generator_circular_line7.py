...
def canAppend2(lst, service2, services):
    ...
    if not timeDur <= (Duty_hours - 25):
        return False

    startEndStnTF = lst[-1].endStn == service2.startStn # checks duty end station and serv start station
    startEndStnTFafterBreak = lst[-1].endStn[:4] == service2.startStn[:4] and lst[-1].endStn[:4] in crewControl #chechks only station, without cosidering direction
...
def allotService(services, start , result):
    ...
    if start == len(services):
        if result:
            ...
            else: totalBreakDur = (short_break + long_break) <= sum(BreakDur) <= 120
            if not totalBreakDur:
                return
            
            if breakInSameJur and drivingDurTF and morningDriveDur and dutyDurTF and longBreak and totalBreakDur:
                tempOutput = []
                global output
                for i in result:
