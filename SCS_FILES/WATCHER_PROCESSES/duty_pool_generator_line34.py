...
# depots_outstations = [str(line) for line in retrieved_lists[3]]
...
class Services:
    def __init__(self, attrs):
        ...
        self.startStn = str(attrs[2])
        self.startTime = hhmm2mins(attrs[3]) # from hh:mm to min
        self.endStn = str(attrs[4])
        self.endTime = hhmm2mins(attrs[5]) # # from hh:mm to min
        self.dir = str(attrs[6]) # up or down (up means towards MKPR)
        self.servDur = int(attrs[7]) # minutes
        try:
            self.stepbackTrainNum = int(attrs[9])
        except ValueError:
...
def find_specific_services(inputFileLocation, outstation_path):
    ...
    try:
        ...
        temp_dsto_services = [int(x) for x in temp_dsto_services if pd.notna(x)]
        # print("Temp Initial RI services: ",temp_ri_services)
        # Check for time differences > 120 minutes
        if len(dsto_df) > 1:
            for i in range(len(dsto_df)-1):
                time1 = pd.to_datetime(dsto_df.iloc[i][3])
                time2 = pd.to_datetime(dsto_df.iloc[i+1][3])
                if (time2 - time1).total_seconds() / 60 > 120:
                    ...
                    valid_dsto_initial_sevice = int(dsto_df.iloc[i][0])
                    break
        invalid_dsto_initial_services = [int(x) for x in temp_dsto_services if x != valid_dsto_initial_sevice]
        # print("Invalid Initial RI services: ", invalid_ri_initial_services)
    except Exception as e:
        print(f"No step back start found for DSTO while processing services: {e}")

    try:
        necc_df = df_mainloops[df_mainloops[2] == 'NECC']
        temp_necc_services = necc_df[0].tolist()
        temp_necc_services = [int(x) for x in temp_necc_services if pd.notna(x)]
        if len(necc_df) > 1:
            for i in range(len(necc_df)-1):
                time1 = pd.to_datetime(necc_df.iloc[i][3])
                time2 = pd.to_datetime(necc_df.iloc[i+1][3])
                if (time2 - time1).total_seconds() / 60 > 120:
                    # print(f"Time difference > 120 minutes between services: {necc_df.iloc[i][0]} and {necc_df.iloc[i+1][0]}")
                    valid_necc_initial_sevice = int(necc_df.iloc[i][0])
                    break
        invalid_necc_initial_services = [int(x) for x in temp_necc_services if x != valid_necc_initial_sevice]
        # print("Invalid Initial necc services: ", invalid_necc_initial_services)
    except Exception as e:
        print(f"No step back start found for NECC while processing services: {e}")
    

    try:
        vasi_df = df_mainloops[df_mainloops[2] == 'VASI']
        temp_vasi_services = vasi_df[0].tolist()
        temp_vasi_services = [int(x) for x in temp_vasi_services if pd.notna(x)]
        if len(vasi_df) > 1:
            for i in range(len(vasi_df)-1):
                time1 = pd.to_datetime(vasi_df.iloc[i][3])
                time2 = pd.to_datetime(vasi_df.iloc[i+1][3])
                if (time2 - time1).total_seconds() / 60 > 120:
                    # print(f"Time difference > 120 minutes between services: {vasi_df.iloc[i][0]} and {vasi_df.iloc[i+1][0]}")
                    valid_vasi_initial_sevice = int(vasi_df.iloc[i][0])
                    break
        invalid_vasi_initial_services = [int(x) for x in temp_vasi_services if x != valid_vasi_initial_sevice]
        # print("Invalid Initial vasi services: ", invalid_vasi_initial_services)
    except Exception as e:
        print(f"No step back start found for VASI while processing services: {e}")
    
    invalid_initial_sevices = invalid_dsto_initial_services + invalid_necc_initial_services + invalid_vasi_initial_services
    ...
    try:
        if len(dsto_df) > 1:
            for i in range(len(dsto_df)-1):
                time1 = hhmm2mins(dsto_df.iloc[i][3])
                time2 = hhmm2mins(dsto_df.iloc[i+1][3])
                if (time1 - time2) > 50:
                    # print(f"Time difference > 120 minutes between services: {ri_df.iloc[i][0]} and {ri_df.iloc[i+1][0]}")
                    valid_dsto_signoff_service = int(dsto_df.iloc[i][0])
                    break
        temp_dsto_services.reverse()
        invalid_dsto_signoff_services = [int(x) for x in temp_dsto_services if x != valid_dsto_signoff_service]
    except Exception as e:
        print(f"No step back signoff found while processing services: {e}")

    necc_df = df_mainloops[df_mainloops[4] == 'NECC']
    necc_df = necc_df.iloc[::-1]  # Reverse the DataFrame to process in reverse order
    temp_necc_services = necc_df[0].tolist()
    temp_necc_services = [int(x) for x in temp_necc_services if pd.notna(x)]
    try:
        if len(necc_df) > 1:
            for i in range(len(necc_df)-1):
                time1 = hhmm2mins(necc_df.iloc[i][3])
                time2 = hhmm2mins(necc_df.iloc[i+1][3])
                if (time1 - time2) > 50:
                    # print(f"Time difference > 120 minutes between services: {ri_df.iloc[i][0]} and {ri_df.iloc[i+1][0]}")
                    valid_necc_signoff_service = int(necc_df.iloc[i][0])
                    break
        temp_necc_services.reverse()
        invalid_necc_signoff_services = [int(x) for x in temp_necc_services if x != valid_necc_signoff_service]
        # print("Invalid RI signoff services: ", invalid_ri_signoff_services)
    except Exception as e:
        print(f"No step back signoff found while processing services: {e}")


    vasi_df = df_mainloops[df_mainloops[4] == 'VASI']
    vasi_df = vasi_df.iloc[::-1]  # Reverse the DataFrame to process in reverse order
    temp_vasi_services = vasi_df[0].tolist()
    temp_vasi_services = [int(x) for x in temp_vasi_services if pd.notna(x)]
    try:
        if len(vasi_df) > 1:
            for i in range(len(vasi_df)-1):
                time1 = hhmm2mins(vasi_df.iloc[i][3])
                time2 = hhmm2mins(vasi_df.iloc[i+1][3])
                if (time1 - time2) > 50:
                    # print(f"Time difference > 120 minutes between services: {ri_df.iloc[i][0]} and {ri_df.iloc[i+1][0]}")
                    valid_vasi_signoff_service = int(vasi_df.iloc[i][0])
                    break
        temp_vasi_services.reverse()
        invalid_vasi_signoff_services = [int(x) for x in temp_vasi_services if x != valid_vasi_signoff_service]
        # print("Invalid RI signoff services: ", invalid_ri_signoff_services)
    except Exception as e:
        print(f"No step back signoff found while processing services: {e}")

    invalid_signoff_services = invalid_vasi_signoff_services + invalid_dsto_signoff_services + invalid_necc_signoff_services
...
def fetchData(data = inputFileLocation):
    ...
    with open(data, "r") as output:
        ...
        for i in range(0, len(tempService)):
            ...
            toAppendTF = temp.trainNum in range(301,311) # for debugging..
...
def canAppend2(lst, service2, services):
    #THESE LINES EXPLICITLY CHECKS FOR TRAIN 394 AND 393 AND 398 AND 399 DUT TO TIMETALBE AND OCC REQUIREMENTS ELSE ITS NOT REQUIRED
    if 394 in all_trainNums and 393 in all_trainNums:
        if( (lst[-1].stepbackTrainNum == 394 and lst[-1].endStn == 'DW DN' and service2.trainNum == 394 and service2.endStn == 'NFD') or (lst[-1].stepbackTrainNum == 393 and lst[-1].endStn == 'DW DN' and service2.trainNum == 393 and service2.endStn == 'NFD') ):
            return False
    # elif (lst[-1].trainNum == 398 and lst[-1].endStn == 'YB DN' and not(service2.trainNum == 398 and service2.startStn == 'YB DN') ) or ( not(lst[-1].trainNum == 398 and lst[-1].endStn == 'YB DN') and service2.trainNum == 398 and service2.startStn == 'YB DN' ) or (lst[-1].trainNum == 399 and lst[-1].endStn == 'YB DN' and not(service2.trainNum == 399 and service2.startStn == 'YB DN') ) or ( not(lst[-1].trainNum == 399 and lst[-1].endStn == 'YB DN') and service2.trainNum == 399 and service2.startStn == 'YB DN'):
    #     return False
    if(lst[-1].trainNum == 398 and lst[-1].endStn == 'NECC' and not(service2.trainNum == 398 and service2.startStn == 'NECC') ) or ( not(lst[-1].trainNum == 398 and lst[-1].endStn == 'NECC') and service2.trainNum == 398 and service2.startStn == 'NECC') or (lst[-1].trainNum == 399 and lst[-1].endStn == 'NECC' and not(service2.trainNum == 399 and service2.startStn == 'NECC') ) or ( not(lst[-1].trainNum == 399 and lst[-1].endStn == 'NECC') and service2.trainNum == 399 and service2.startStn == 'NECC'):
        return False
    elif( (lst[-1].trainNum == 398 and lst[-1].endStn == 'YB UP' and service2.trainNum == 398 and service2.startStn == 'YB UP') or (lst[-1].trainNum == 399 and lst[-1].endStn == 'YB UP' and service2.trainNum == 399 and service2.startStn == 'YB UP') ):
        return False
    elif (lst[-1].trainNum == 398 and lst[-1].endStn == 'DW UP' and service2.trainNum == 398 and service2.startStn == 'DW UP') or (lst[-1].trainNum == 399 and lst[-1].endStn == 'DW UP' and service2.trainNum == 399 and service2.startStn == 'DW UP'):
        return False
    elif(lst[-1].trainNum == 398 and lst[-1].endStn == 'DSTO' and not(service2.trainNum == 398 and service2.startStn == 'DSTO') ) or ( not(lst[-1].trainNum == 398 and lst[-1].endStn == 'DSTO') and service2.trainNum == 398 and service2.startStn == 'DSTO') or (lst[-1].trainNum == 399 and lst[-1].endStn == 'DSTO' and not(service2.trainNum == 399 and service2.startStn == 'DSTO') ) or ( not(lst[-1].trainNum == 399 and lst[-1].endStn == 'DSTO') and service2.trainNum == 399 and service2.startStn == 'DSTO'):
        return False
    elif ((lst[-1].trainNum == 398 and lst[-1].endStn == 'DW DN' and service2.trainNum == 398 and service2.startStn == 'DW DN') or (lst[-1].trainNum == 399 and lst[-1].endStn == 'DW DN' and service2.trainNum == 399 and service2.startStn == 'DW DN') ):
        return False
    # elif(lst[-1].trainNum == 398 and lst[-1].endStn == 'DW UP' and not(service2.trainNum == 398 and service2.startStn == 'DW DN') ) or ( not(lst[-1].trainNum == 398 and lst[-1].endStn == 'DW UP') and service2.trainNum == 398 and service2.startStn == 'DW DN') or (lst[-1].trainNum == 399 and lst[-1].endStn == 'DW UP' and not(service2.trainNum == 399 and service2.startStn == 'DW DN') ) or ( not(lst[-1].trainNum == 399 and lst[-1].endStn == 'DW UP') and service2.trainNum == 399 and service2.startStn == 'DW DN'):
    #     return False
    else:
        ...
        if not timeDur <= (Duty_hours - 25): # checks duty hours
            return False

        startEndStnTF = lst[-1].endStn == service2.startStn # checks duty end station and serv start station
        startEndStnTFafterBreak = lst[-1].endStn[:3] == service2.startStn[:3] and lst[-1].endStn != 'YB SDG' and lst[-1].endStn != 'YBD' and service2.startStn != 'YB SDG' and service2.startStn != 'YBD' #chechks only station, without cosidering direction
        ...
        if startEndRakeTF:
            if startEndStnTF and startEndTimeTF:
                ...
                for service in reversed(lst):
                    ...
                    #         contTimeDur = service2.endTime - service.startTime
                    #         break
                if contTimeDur <= Continuous_Driving_time:
                    return True
                else: return False
            else: return False

        elif startEndTimeWithin and lst[-1].endStn in crewControl and service2.startStn in crewControl:
...
def allotService(services, start , result):
    global count
    """this function take the list of services as input and start making all the
    ...
    if start == len(services):
        if result:
            if result[-1].servNum in invalid_signoff_services:
                return
            if juris_conflict == 0 :
            ...
            for i in range(len(result)-1):
                if (result[i+1].startTime - result[i].endTime) >=short_break:
                    ...
                    if not firstBreak:
                        firstBreak = True
                        if (result[i].endTime - result[0].startTime) > 30:
            ...
            if (result[0].servNum in induction_services) and (result[-1].servNum in stabling_services): # if duty starts before 6:15 or ends after 11:30
                ...
                s_off_time = result[-1].endTime + 10
                s_on_off_margin = 25
                print("There is some error in dutyDurTF\n", result[0].servNum,'\n', result[-1].servNum)
            ...
            if 300 < s_on_time <= 360 or 1410 <= s_off_time < 1440:
                dutyDurTF = dutyDur <= (Duty_hours - 30 - s_on_off_margin)
            elif s_on_time <= 300 or s_off_time >= 1440:
                dutyDurTF = dutyDur <= (Duty_hours - 30 - s_on_off_margin)
            # ---------------------------------------------------------------
            ...
            if breakInSameJur and dutyDurTF and drivingDurTF and longBreak and morningDriveDur:
                #output.append(result[::])
                tempOutput = []
                global output
                for i in result:
...
all_trainNums = []
for s in servicesLstOrig:
    all_trainNums.append(s.trainNum)
all_trainNums = list(set(all_trainNums))  # Remove duplicates
...
# print("Induction Services:", induction_services)
# print("Stabling Services:", stabling_services)
# print("Invalid Initial Services:", invalid_initial_services)
# print("Invalid Signoff Services:", invalid_signoff_services)
...
# output_file = f'/mnt/c/Users/Yogesh Gangil/Desktop/DMRC IITB/crew_line34/CODE FOR TT WEEKDAYS NEW 6/USEFUL OUTPUT/output_new_{initial_start}_{final_end}.csv'
# open(output_file, 'w').close()

allotService(servicesLstOrig, initial_start, result)  # change start value for 
