...
df = pd.read_excel(f"{inputFile}",sheet_name=f"Sheet1",header = None)
...
stablingRakes = []
# uniqueID = []
reqIndexofStn = [56,71,73,94,99]
reqIndexofStn1 = []


for i in range(1,df.shape[1]):
  if not pd.isna(df.iloc[54,i]) and type(df.iloc[54,i]) != str :  ## 4,i for line 3,4 TT
    inductingRakes.append(df.iloc[55,i])  # 1,i for line 3,4 TT
    RakeNum.append(df.iloc[55,i])     # rakenum 1,i for line 3,4 TT
    startStn.append(df.iloc[53,i])
    startTime.append(df.iloc[54,i].strftime('%H:%M'))
    ...
    if flag: list1 = reqIndexofStn
      ...
      startTime.pop()
      RakeNum.pop()
      print(f"error1: end station not found for train = {df.iloc[55,i]} at row = {j} and column = {i}")

    for j in list1:
      if j >= req:
        if not pd.isna(df.iloc[j,i]) and type(df.iloc[j,i]) != str and type(df.iloc[j,i]) != int:
          if j != list1[-1]:
            RakeNum.append(df.iloc[55,i])
        ...
        if not pd.isna(df.iloc[j,i]) and type(df.iloc[j,i]) != str and type(df.iloc[j,i]) != int:
          ...
          for k in reqIndexofStn1:
            if not pd.isna(df.iloc[k,i]) and type(df.iloc[k,i]) != str and type(df.iloc[k,i]) != int:
              ...
              break
          if flagX: break
          else: print("error-X",df.iloc[55,i],df.iloc[6,i])
      for j in reqIndexofStn1:
        if j >= req:
          if not pd.isna(df.iloc[j,i]) and type(df.iloc[j,i]) != str and type(df.iloc[j,i]) != int:
    ...
    for j in range(56,100):
      ...
      if not pd.isna(df.iloc[j,i]) and type(df.iloc[j,i]) != str and type(df.iloc[j,i]) != int:
        ...
        if flag2: break
          ...
          print(f"error2: end station not found for train = {df.iloc[55,i]} at row = {j} and column = {i}")
    if 'list2' in locals():
      for j in list2:
        if j >= req:
          if not pd.isna(df.iloc[j,i]) and type(df.iloc[j,i]) != str and type(df.iloc[j,i]) != int:
              ...
              if not pd.isna(df.iloc[list2[k+1],i]) and type(df.iloc[list2[k+1],i]) != str:
                ...
                for j in range(list2[k+1],list2[k]-1,-1):
                  ...
                  if not pd.isna(df.iloc[j,i]) and type(df.iloc[j,i]) != str:
                    ...
                    flag3 = True
                    break
                if not flag3: print("error3", df.iloc[55,i])
  ...
  if not pd.isna(df.iloc[102,i]) and type(df.iloc[102,i]) != str:# and 'SVVR' not in df.iloc[85,i]:
    RakeNum.append(df.iloc[55,i])
    startStn.append(endStn[-1])
    startTime.append(endTime[-1])
    endStn.append(df.iloc[101,i])
    endTime.append(df.iloc[102,i].strftime('%H:%M'))
    direction.append('UP')
...
services.to_csv(f"{outputFileLocation}InitialServices_up.csv")
