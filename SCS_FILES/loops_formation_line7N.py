  ...
  if df.iloc[i,0] not in added_services and df.iloc[i,1] not in svvrRake:
    if df.iloc[i,4] == "IPE":
      for j in range(i, df.shape[0]):
        if df.iloc[j,2] == "IPE" and df.iloc[j,1] not in svvrRake and df.iloc[j,0] not in added_services and df.iloc[i,1] == df.iloc[j,1] and timeDiff(df.iloc[i,5], df.iloc[j,3]) < 14:
...
for i in range(df.shape[0]):
  if df.iloc[i,0] not in added_services and df.iloc[i,1] not in svvrRake:
    if df.iloc[i,4] == "MKPR":
      flag = False
      for j in range(i, df.shape[0]):
        if df.iloc[j,2] == "MKPR" and df.iloc[i,1] == df.iloc[j,1] and timeDiff(df.iloc[i,5], df.iloc[j,3]) < 30 and df.iloc[j,0] not in added_services and df.iloc[j,1] not in svvrRake:
          #print(df.iloc[i,1], df.iloc[i,2], df.iloc[i,3], df.iloc[i,4], df.iloc[i,5] , "with", df.iloc[j,1], df.iloc[j,2], df.iloc[j,3], df.iloc[j,4], df.iloc[j,5])
          added_services.append(df.iloc[i,0])
          added_services.append(df.iloc[j,0])
          rakeNum.append(df.iloc[i,1])
          ...
          flag = True
          break
      if not flag: print("No Same train Found at MKPR", df.iloc[i,0],df.iloc[i,1], df.iloc[i,2], df.iloc[i,3], df.iloc[i,4], df.iloc[i,5])
...
for i in range(df.shape[0]):
  if df.iloc[i,0] not in added_services and df.iloc[i,1] not in svvrRake:
    if df.iloc[i,4] == "SVVR" :
      flag = False
      for j in range(i, df.shape[0]):
        if df.iloc[j,2] == "SVVR" and  timeDiff(df.iloc[i,5], df.iloc[j,3]) < 20 and df.iloc[j,0] not in added_services and df.iloc[j,1] not in svvrRake and df.iloc[i,1] == df.iloc[j,1]:
          added_services.append(df.iloc[i,0])
          added_services.append(df.iloc[j,0])
          rakeNum.append(df.iloc[i,1])
          startStn.append(df.iloc[i,2])
          ...
          flag = True
          break
      if not flag: print("No Same train Found at SVVR", df.iloc[i,0],df.iloc[i,1], df.iloc[i,2], df.iloc[i,3], df.iloc[i,4], df.iloc[i,5])
...
for i in range(df.shape[0]):
  if df.iloc[i,0] not in added_services and df.iloc[i,1] not in svvrRake:# and 420 <= hhmm2mins(df.iloc[i,5]) <= 1250:
    if df.iloc[i,2] == 'KKDA DN' and df.iloc[i,4] == "MUPR":
      flag = False
      for j in range(i, df.shape[0]):
        if df.iloc[j,2] == "MUPR" and df.iloc[j,4] == 'KKDA UP' and df.iloc[j,1] == df.iloc[i,1] and timeDiff(df.iloc[i,5], df.iloc[j,3]) <= 15 and df.iloc[j,0] not in added_services and df.iloc[j,1] not in svvrRake:
