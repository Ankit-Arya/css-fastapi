...
def main():
    ...
    try:
        ...
        # df_final.to_csv(r'redefinedinputparameters.csv', index=False)
        ...
        #     if short_break <= gap <= 120 and s1.endStn[:4] in crewControlSet: # check s1.endStn[:4] for DWT 
        #         return s1.endStn[:2] == s2.startStn[:2] and (s2.endTime - s1.startTime) <= Duty_hours
        #     return False

        # # ============ Continuous Driving Time Checker ============
        ...
        # # =========== Additional SOP Constraints Checker ============

        # def check_additional_constraints(path):
        #     # 1. Ensure +10 min additional break after DWT for TO to ensure cab charge is taken
        #     for i in range(len(path) - 1):
        #         if path[i].endStn == "DWT":
        #             gap = path[i + 1].startTime - path[i].endTime
        #             # only allow 40–49 inclusive, or anything above 60
        #             if not (40 <= gap <= 49 or gap > 60):
        #                 return False

        #     # 2. Ensure ≥40 min break after ≥120 min first continuous driving
        #     cont_drive = path[0].endTime - path[0].startTime
        #     for i in range(1, len(path)):
        #         gap = path[i].startTime - path[i - 1].endTime
        #         if gap >= short_break:
        #             if cont_drive >= 120 and gap < 40:
        #                 return False
        #             break
        #         else:
        ...
        # # =========== Long Continuous Driving Check ============

        # def check_135_min_continuous_breaks(path, min_continuous=140, required_break=40, short_break=30):
        #     cont_drive = path[0].endTime - path[0].startTime
        #     for i in range(1, len(path)):
        #         prev = path[i - 1]
        #         curr = path[i]
        #         gap = curr.startTime - prev.endTime
        #         if gap >= short_break:
        #             if cont_drive >= min_continuous and gap < required_break:
        #                 return False
        #             cont_drive = curr.endTime - curr.startTime
        #         else:
        #             cont_drive += curr.endTime - prev.endTime
        ...
        #         for i in range(len(path) - 1):
        #             gap = path[i + 1].startTime - path[i].endTime
        #             # added on 21-07-2025 - it'll drop duties where train coming from dsto to dwdn goes nfd in continuation in peak hours to stop bunching
        #             nxt = path[i+1]
        #             # ─── NEW RULE ───
        #             if (
        ...
        #         )
        #         # --- NEW CONSTRAINT: before 06:00, total servDur < 330 min ---
        #         if start_time < 6 * 60:
        #             total_trip_duration = sum(s.servDur for s in path)
        #             if total_trip_duration >= 330:
        #                 # too long for an early (pre-6 AM) duty
        #                 continue
        # # -----------------------------------------------------------
        #         # ─── NEW BLOCK added 28-07-2025 ───
        #         # 1) decide how long *this* first break must be
        #         first_break_threshold = 80 if path[0].trainNum in (398, 399, '398','399') else 40
        #         # print('first_break_threshold and trainNum',first_break_threshold,path[0].trainNum)

        #         # 2) walk until you hit that break
        #         continuous_end = path[0].endTime
        #         for i in range(1, len(path)):
        #             gap = path[i].startTime - path[i-1].endTime
        #             if gap < first_break_threshold:
        #                 # still part of the “continuous” chunk
        #                 continuous_end = path[i].endTime
        #             else:
        #                 # this gap is our first *real* break
        #                 break

        #         initial_cont = continuous_end - path[0].startTime
        #         # print('initial_cont',initial_cont)
        #         # 3) enforce minimum continuous‐trip length
        #         first_cont_trip = (initial_cont <= 30)

        #         # ─── PHASE 2: collect all positive gaps as “breaks” ────────
        #         breaks = []
        #         if has_long_break:
        #             for i in range(len(path)-1):
        #                 gap = path[i+1].startTime - path[i].endTime
        ...
        # # ============ Single-thread Duty Generation ============

        # def fast_duty_generation_single_threaded(services):
        ...
        #         duties = generate_duties(service, graph, cc1, cc2)
        #         print(f"[{i+1}/{len(services)}] Service {service.servNum}: {len(duties)} duties generated.")
        ...
        #         max_depth = max(len(duty) for duty in duty_pool)
        ...
        #     print("Generating duty pool (single-threaded)...")
        #     duty_pool = fast_duty_generation_single_threaded(services)
        ...
        #     print("✅ Done.")
            ...
            # [Your subsequent processing code can go here]
